...
This file defines the coding standards, project conventions, and best practices that Codex must
...
**RagPrincipals** is an experimental sandbox repository for exploring Codex's
capabilities and potential. It serves as a playground for testing AI-assisted development
workflows, automation patterns, and Codex integrations — built with Python.
...
- Include a link to the Codex session at the end of each commit message (automatically
  appended by Codex).
- Feature branches follow the pattern: `Codex/<description>-<session-id>`
- Always push with: `git push -u origin <branch-name>`
- Branch names must start with `Codex/` and end with the matching session ID, or the push
...
- **Model in use**: `Codex-sonnet-4-6` (default); switch to `Codex-opus-4-6` for complex
  tasks or `Codex-haiku-4-5` for fast/cheap tasks.
- **Hooks**: Codex supports `SessionStart` and other lifecycle hooks — add them under
  `.Codex/hooks/` as experiments are built.
...
- **Active branch**: `Codex/update-Codex-python-standards-iPJNH`
